...
const ExpenseSplitter = () => {
  ...
  const initClient = useCallback(() => {
    ...
    console.log('initClient: Checking for window.gapi.client and window.gapi.auth2. window.gapi.auth2 is:', window.gapi && window.gapi.auth2);
    if (!window.gapi || !window.gapi.client || !window.gapi.auth2) {
      console.error('initClient: Prerequisite GAPI objects (client or auth2) not available. Returning.');
      setExportStatus("❌ Google API not loaded yet. Please wait or refresh.");
      setTimeout(() => setExportStatus(''), 5000);
      return;
    }
    console.log('initClient: Attempting window.gapi.client.init()...');
    window.gapi.client.init({
      ...
      console.log('initClient: window.gapi.client.init() SUCCEEDED.');
      const instance = window.gapi.auth2.getAuthInstance();
      console.log('initClient: Attempting to setAuthInstance.');
      setAuthInstance(instance);
      console.log('initClient: setAuthInstance CALLED.');
      instance.isSignedIn.listen(updateSigninStatus);
      updateSigninStatus(instance.isSignedIn.get());
      console.log('initClient: Attempting window.gapi.client.load(\'drive\', \'v3\')...');
      window.gapi.client.load('drive', 'v3').then(() => {
        console.log('initClient: window.gapi.client.load(\'drive\', \'v3\') SUCCEEDED.');
      }).catch(err => {
        console.error('initClient: window.gapi.client.load(\'drive\', \'v3\') FAILED:', err);
      ...
      console.error('initClient: window.gapi.client.init() FAILED:', error);
  ...
  useEffect(() => {
    const checkGapiReady = () => {
      console.log('checkGapiReady: window.gapi:', window.gapi, 'window.gapi.load:', window.gapi && window.gapi.load, 'window.gapi.client:', window.gapi && window.gapi.client);
      if (window.gapi && window.gapi.load) { // Condition changed here
        console.log('checkGapiReady: Attempting window.gapi.load(\'client:auth2\')...');
        window.gapi.load('client:auth2', function() {
            console.log('checkGapiReady: window.gapi.load for client:auth2 CALLED BACK. Now calling initClient.');
            initClient();
        });
      } else {
            ...
            <div className="flex gap-2 flex-wrap items-center"> {/* Added items-center for alignment */}
              {isAuthenticated && userInfo ? (
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors ${!authInstance ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <LogIn size={18} />
                  Sign in with Google
                </button>
