...
const ExpenseSplitter = () => {
  ...
  const handleLoadFromDrive = async () => {
    if (!isAuthenticated || !gapi.client.drive) {
  ...
  useEffect(() => {
    ...
    // Load the GAPI script if it's not already loaded (gapi-script handles this implicitly)
    // and then initialize.
    // The gapi-script library should handle loading the gapi object onto the window.
    // We just need to ensure client:auth2 is loaded before initializing.
    if (window.gapi) { // Check if gapi is already loaded by gapi-script
        gapi.load('client:auth2', initClient);
    } else {
        // This case might not be strictly necessary if gapi-script guarantees gapi is available
        // but as a fallback:
